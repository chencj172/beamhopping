...
# 保存数据（同时保存为.npy和CSV格式）
# 保存为.npy格式（兼容性考虑）
satellite_data.save_to_file('results/satellite_position_fixed.npy')
cell_data.save_to_file('results/cell_positions_fixed.npy')
traffic_data.save_to_file('results/traffic_data_poisson.npy')

# 保存为CSV格式（按用户要求）
satellite_data.save_to_csv('results/satellite_position_fixed.csv')
cell_data.save_to_csv('results/cell_positions_fixed.csv')
traffic_data.save_to_csv('results/traffic_data_poisson.csv')

# 可视化卫星位置
print("可视化卫星位置...")
satellite_data.plot_orbit()
plt.savefig('results/satellite_position_fixed.png')
plt.close()

# 可视化小区分布
print("可视化小区分布...")
cell_data.plot_cells_2d()
plt.savefig('results/cell_distribution_2d_fixed.png')
plt.close()

cell_data.plot_cells_3d(satellite_data.get_position())
plt.savefig('results/cell_distribution_3d_fixed.png')
plt.close()

# 可视化业务请求
print("可视化业务请求...")
traffic_data.plot_traffic_patterns([0, 5, 10, 15])
plt.savefig('results/traffic_patterns_poisson.png')
plt.close()

traffic_data.plot_traffic_heatmap(43200)  # 中午12点的业务请求（12小时 = 43200秒）
plt.savefig('results/traffic_heatmap_poisson.png')
plt.close()
...
# 保存模型
agent.save('models/ppo_model_fixed.pt')
...
Visualization.plot_learning_curve(
    ...
    avg_window=1,
    title='Learning Curve (Fixed Satellite Position)'
)
plt.savefig('results/learning_curve_fixed.png')
plt.close()
...
Visualization.plot_performance_metrics(
    ...
    title='Performance Metrics (Fixed Satellite Position)'
)
plt.savefig('results/performance_metrics_fixed.png')
plt.close()
...
Visualization.plot_beam_allocation(
    ...
    title='Beam Allocation (Fixed Satellite Position)'
)
plt.savefig('results/beam_allocation_fixed.png')
plt.close()
...
plt.savefig('results/sinr_heatmap_fixed.png')
plt.close()
...
plt.savefig('results/queue_status_fixed.png')
plt.close()
